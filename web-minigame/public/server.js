...
function requestHandler(req, res) {
  ...
  // -------------------------------------------------------------
  // 3. POLL API: GET /api/rooms/:code/poll?since=<timestamp> (Vercel fallback)
  // Returns all buffered messages newer than ?since= (for environments without SSE)
  // -------------------------------------------------------------
  const pollMatch = cleanUrl.match(/^\/api\/rooms\/([A-Za-z0-9_-]+)\/poll$/i);
  if (pollMatch) {
    if (req.method !== 'GET') {
      res.writeHead(405, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({ success: false, error: 'Method Not Allowed' }));
    ...
    const code = pollMatch[1].toUpperCase();
    let since = 0;
    try {
      const urlObj = new URL(req.url || '/', 'http://localhost');
      since = parseInt(urlObj.searchParams.get('since') || '0', 10) || 0;
    } catch(e) {}

    const buf = roomMessageBuffers.get(code) || [];
    // Return only messages newer than `since`, up to last 10 on first load
    const msgs = since === 0 ? buf.slice(-10) : buf.filter(m => (m._serverTime || 0) > since);
    ...
    res.end(JSON.stringify({ success: true, messages: msgs, serverTime: Date.now() }));
    return;
  }

  // -------------------------------------------------------------
  // 4. GAME STATE SYNC API: GET & POST /api/rooms/:code/state
  // -------------------------------------------------------------
  ...
  fs.stat(filePath, (err, stats) => {
    ...
    if (!fileFound) {
      ...
      if (publicPath.startsWith(__dirname) && fs.existsSync(publicPath) && fs.statSync(publicPath).isFile()) {
        ...
        // SPA Fallback to index.html (do not fallback for missing assets)
